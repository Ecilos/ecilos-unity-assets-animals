...
public class lb_BirdController : MonoBehaviour {
  ...
  public void ChangeCamera(Camera cam) { currentCamera = cam; }
  ...
  void Start() {
    ...
    for (int i = 0; i < myBirds.Length; i++) {
      if (highQuality) {
        bird = Resources.Load(myBirdTypes[Random.Range(0, myBirdTypes.Count)] +
                                  "HQ",
      ...
      // @todo: Handle exceptions: ArgumentException: The Object you want to
      // instantiate is null.
      myBirds[i] =
          Instantiate(bird, Vector3.zero, Quaternion.identity) as GameObject;
    ...
    GameObject[] groundTargets =
        GameObject.FindGameObjectsWithTag("lb_groundTarget");
    GameObject[] perchTargets =
        GameObject.FindGameObjectsWithTag("lb_perchTarget");

    for (int i = 0; i < groundTargets.Length; i++) {
      if (Vector3.Distance(groundTargets[i].transform.position,
    ...
    for (int i = 0; i < perchTargets.Length; i++) {
      if (Vector3.Distance(perchTargets[i].transform.position,
                           currentCamera.transform.position) <
          unspawnDistance) {
    ...
    GameObject fEmitter =
        Resources.Load("featherEmitter", typeof(GameObject)) as GameObject;
    for (int i = 0; i < 3; i++) {
      featherEmitters[i] = Instantiate(fEmitter, Vector3.zero,
  ...
  Vector3 FindPointInGroundTarget(GameObject target) {
    // find a random point within the collider of a ground target that touches
    // the ground
    Vector3 point;
    ...
    if (Physics.Raycast(point, -Vector3.up, out hit,
                        target.GetComponent<Collider>().bounds.size.y,
                        groundLayer)) {
      return hit.point;
    }
  ...
  void UpdateBirds() {
    ...
    if (activeBirds < idealNumberOfBirds && AreThereActiveTargets()) {
      ...
      SpawnBird();
    } else if (activeBirds < maximumNumberOfBirds && Random.value < .05 &&
               AreThereActiveTargets()) {
      // if there are less than maximum birds active spawn a bird every 20
      // seconds
      SpawnBird();
    }

    // check one bird every second to see if it should be unspawned
    if (myBirds[birdIndex].activeSelf &&
        BirdOffCamera(myBirds[birdIndex].transform.position) &&
        Vector3.Distance(myBirds[birdIndex].transform.position,
                         currentCamera.transform.position) > unspawnDistance) {
      // if the bird is off camera and at least unsapwnDistance units away lets
      // unspawn
  ...
  // this function will cycle through targets removing those outside of the
  // unspawnDistance it will also add any new targets that come into range
  IEnumerator UpdateTargets() {
    ...
    while (true) {
      ...
      for (int i = 0; i < birdGroundTargets.Count; i++) {
        if (Vector3.Distance(birdGroundTargets[i].transform.position,
                             currentCamera.transform.position) >
            unspawnDistance) {
      ...
      for (int i = 0; i < birdPerchTargets.Count; i++) {
        if (Vector3.Distance(birdPerchTargets[i].transform.position,
                             currentCamera.transform.position) >
            unspawnDistance) {
      ...
      Collider[] hits = Physics.OverlapSphere(currentCamera.transform.position,
                                              unspawnDistance);
      foreach (Collider hit in hits) {
        if (hit.tag == "lb_groundTarget" &&
            !birdGroundTargets.Contains(hit.gameObject)) {
          birdGroundTargets.Add(hit.gameObject);
        }
        if (hit.tag == "lb_perchTarget" &&
            !birdPerchTargets.Contains(hit.gameObject)) {
  ...
  void SpawnBird() {
    if (!pause) {
      ...
      while (bird == null) {
        ...
        randomBirdIndex =
            randomBirdIndex + 1 >= myBirds.Length ? 0 : randomBirdIndex + 1;
        loopCheck++;
  ...
  Vector3 FindPositionOffCamera() {
    ...
    ray += new Vector3(Random.Range(-.5f, .5f), Random.Range(-.5f, .5f),
                       Random.Range(-.5f, .5f));
    // cycle through random rays until we find one that doesnt hit anything
    while (
        Physics.Raycast(currentCamera.transform.position, ray, out hit, dist)) {
  ...
  void BirdFindTarget(GameObject bird) {
    ...
    GameObject target;
    if (birdGroundTargets.Count > 0 || birdPerchTargets.Count > 0) {
      // pick a random target based on the number of available targets vs the
      // area of ground targets each perch target counts for .3 area, each
      // ground target's area is calculated
      float gtArea = 0.0f;
      float ptArea = birdPerchTargets.Count * 0.3f;

      for (int i = 0; i < birdGroundTargets.Count; i++) {
        gtArea += birdGroundTargets[i].GetComponent<Collider>().bounds.size.x *
                  birdGroundTargets[i].GetComponent<Collider>().bounds.size.z;
      }
      if (ptArea == 0.0f || Random.value < gtArea / (gtArea + ptArea)) {
        target = birdGroundTargets[Mathf.FloorToInt(
            Random.Range(0, birdGroundTargets.Count))];
        bird.SendMessage("FlyToTarget", FindPointInGroundTarget(target));
      } else {
        target = birdPerchTargets[Mathf.FloorToInt(
            Random.Range(0, birdPerchTargets.Count))];
        bird.SendMessage("FlyToTarget", target.transform.position);
      }
    } else {
      bird.SendMessage("FlyToTarget", currentCamera.transform.position +
