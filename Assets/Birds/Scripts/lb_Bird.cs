...
public class lb_Bird : MonoBehaviour {
  ...
  void OnEnable() {
    ...
    // hopForwardAnimationHash = Animator.StringToHash ("Base
    // Layer.hopForward"); hopBackwardAnimationHash = Animator.StringToHash
    // ("Base Layer.hopBack"); hopLeftAnimationHash = Animator.StringToHash
    // ("Base Layer.hopLeft"); hopRightAnimationHash = Animator.StringToHash
    // ("Base Layer.hopRight"); worriedAnimationHash = Animator.StringToHash
    // ("Base Layer.worried"); landingAnimationHash = Animator.StringToHash
    // ("Base Layer.landing");
  ...
  IEnumerator FlyToTarget(Vector3 target) {
    ...
    // birds fly up and away from their perch for 1 second before orienting to
    // the next target
    GetComponent<Rigidbody>().AddForce(
        (transform.forward * 50.0f * controller.birdScale) +
        (transform.up * 100.0f * controller.birdScale));
    float t = 0.0f;
    while (t < 1.0f) {
      if (!paused) {
        t += Time.deltaTime;
    ...
    // if the target is directly above the bird the bird needs to fly out before
    // going up this should stop them from taking off like a rocket upwards
    if (vectorDirectionToTarget.y > .5f) {
      tempTarget = transform.position +
                   (new Vector3(transform.forward.x, .5f, transform.forward.z) *
                    distanceToTarget);
      ...
      while (vectorDirectionToTarget.y > .5f) {
        ...
        transform.rotation =
            Quaternion.Slerp(startingRotation, finalRotation, t);
        anim.SetFloat(
            flyingDirectionHash,
            FindBankingAngle(transform.forward, vectorDirectionToTarget));
        t += Time.deltaTime * 0.5f;
        GetComponent<Rigidbody>().AddForce(
            transform.forward * 70.0f * controller.birdScale * Time.deltaTime);

        // Debug.DrawRay (transform.position,transform.forward,Color.green);

        vectorDirectionToTarget =
            (target - transform.position)
                .normalized; // reset the variable to reflect the actual target
                             // and not the temptarget

        if (Physics.Raycast(transform.position, -Vector3.up, out hit,
                            0.15f * controller.birdScale) &&
            GetComponent<Rigidbody>().velocity.y < 0) {
          // if the bird is going to collide with the ground zero out vertical
          // velocity
          if (!hit.collider.isTrigger) {
            GetComponent<Rigidbody>().velocity =
                new Vector3(GetComponent<Rigidbody>().velocity.x, 0.0f,
        ...
        if (Physics.Raycast(transform.position, Vector3.up, out hit,
                            0.15f * controller.birdScale) &&
            GetComponent<Rigidbody>().velocity.y > 0) {
          // if the bird is going to collide with something overhead zero out
          // vertical velocity
          if (!hit.collider.isTrigger) {
            GetComponent<Rigidbody>().velocity =
                new Vector3(GetComponent<Rigidbody>().velocity.x, 0.0f,
        ...
        // check for collisions with non trigger colliders and abort flight if
        // necessary
        if (controller.collideWithObjects) {
          forwardStraight = transform.forward;
          forwardStraight.y = 0.0f;
          // Debug.DrawRay
          // (transform.position+(transform.forward*.1f),forwardStraight*.75f,Color.green);
          if (Physics.Raycast(transform.position + (transform.forward * .15f *
                                                    controller.birdScale),
                              forwardStraight, out hit,
                              .75f * controller.birdScale)) {
            if (!hit.collider.isTrigger) {
              AbortFlyToTarget();
            }
          }
    ...
    while (true) {
      if (!paused) {
        ...
        GetComponent<Rigidbody>().AddForce(transform.forward * flyingForce *
                                           Time.deltaTime);
        distanceToTarget = Vector3.Distance(transform.position, target);
    ...
    Collider[] hitColliders =
        Physics.OverlapSphere(target, 0.05f * controller.birdScale);
    ...
    // this while loop will reorient the rotation to vertical and translate the
    // bird exactly to the target
    startingRotation = transform.rotation;
    transform.localEulerAngles =
        new Vector3(0.0f, transform.localEulerAngles.y, 0.0f);
    finalRotation = transform.rotation;
    transform.rotation = startingRotation;
    while (distanceToTarget > 0.05f * controller.birdScale) {
      if (!paused) {
        transform.rotation =
            Quaternion.Slerp(startingRotation, finalRotation, t * 4.0f);
        transform.position =
            Vector3.SmoothDamp(transform.position, target, ref vel, 0.5f);
        t += Time.deltaTime;
        distanceToTarget = Vector3.Distance(transform.position, target);
        if (t > 2.0f) {
  ...
  // Sets a variable between -1 and 1 to control the left and right banking
  // animation
  ...
  void OnGroundBehaviors() {
    ...
    if (idle) {
      // the bird is in the idle animation, lets randomly choose a behavior
      // every 3 seconds
      if (Random.value < Time.deltaTime * .33) {
        ...
        // lets alter the agitation level of the brid so it uses a different mix
        // of idle animation next time
  ...
  void OnTriggerExit(Collider col) {
    ...
    if (onGround &&
        (col.tag == "lb_groundTarget" || col.tag == "lb_perchTarget")) {
      FlyAway();
    }
  }
  ...
  void Flee() {
    if (!dead) {
      ...
      farAwayTarget +=
          new Vector3(Random.Range(-100, 100) * controller.birdScale,
                      10 * controller.birdScale,
                      Random.Range(-100, 100) * controller.birdScale);
  ...
  void SetController(lb_BirdController cont) { controller = cont; }

  void ResetHopInt() { anim.SetInteger(hopIntHash, 0); }
