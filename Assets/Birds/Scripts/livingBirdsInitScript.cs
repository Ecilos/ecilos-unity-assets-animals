...
public class livingBirdsInitScript : MonoBehaviour {
  ...
  void Start() {
    ...
    birdControl =
        GameObject.Find("BirdsController").GetComponent<lb_BirdController>();
    SpawnSomeBirds();
  }
  ...
  void Update() {
    if (Input.GetKey(KeyCode.D) || Input.GetKey(KeyCode.RightArrow)) {
      camera1.transform.localEulerAngles +=
          new Vector3(0.0f, 90.0f, 0.0f) * Time.deltaTime;
    }
  ...
  void OnGUI() {
    ...
    if (cameraDirections) {
      ...
      GUI.Label(new Rect(170, 30, 1014, 20),
